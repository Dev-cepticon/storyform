...
export default class StoryformRaceSheet
  ...
  // Logic used by EVERY item in the system
  static async _onToggleEditMode(event, target) {
    const val = this.document.system.config.editMode;
    return this.document.update({ "system.config.editMode": !val });
  }

  static async _onToggleSidebar(event, target) {
    const val = this.document.system.config.showSidebar;
    return this.document.update({ "system.config.showSidebar": !val });
  }
